...
import {getAppid,defAuth,UserAuth,getSignPackage,shareReturn,getActShareInfo,getCurShareInfo} from 'api/wixin'
...
export const wixi_fen = (url, activity_id,das) =>{
        ...
        var url = url, activity_id = activity_id, das=das;
        var wx_title,
            wx_desc,
            wx_url ,
            activityid,
...
//      	if(activity_id == 3){
//      		wx_title = '全民加息+1%'; //分享标题
//      		wx_desc = '[银蝶理财]多种期限标的任你选，所有产品年化收益+1%。';  //分享内容
//      	}else if(activity_id == 4){
//      		wx_title = '新手专享+3%'; //分享标题
//      		wx_desc = '[银蝶理财]新手福利，28天定期产品年化收益+3%。';  //分享内容
//      	}else{
...
//      	wx_url = 'http://dev.yindies.com/devWap/mApp/#/weixinlogin '; //分享链接  测试
//          wx_url = 'http://www.yindies.com/wap/#/weixinlogin'; //分享链接
            getCurShareInfo().then((r)=>{
            	if(r.code == 1000){
        ...
        function getReturn(num){
            shareReturn(num,activity_id).then((r)=>{
                    if(r.code==1000){
                        console.log('分享成功')
                    }else{
                        console.log('分享取消')
